// aui/packages/swipeout/index.js
/*
* touch事件判断方式
...
Component({
  externalClasses: ['aui-class', 'aui-item-class'],
  properties: {
    actions: {
      value: [],
      type: Array,
  ...
  methods: {
    ...
    _updateButtonSize() {
      ...
      if (actions.length > 0) {
        ...
        if (/\d+rpx$/.test(this.data.operateWidth)) {
          wx.getSystemInfo({
            success: function (res) {
              self.data.limitMove = (Number(self.data.operateWidth.replace(/rpx$/, '')) / (750 / res.windowWidth))
            }
          })
        } else if(/\d+px$/.test(this.data.operateWidth)) {
          this.data.limitMove = Number(this.data.operateWidth.replace(/px$/, ''))
        } else {
          this.data.limitMove = Number(this.data.operateWidth)
        }
      }
    },
